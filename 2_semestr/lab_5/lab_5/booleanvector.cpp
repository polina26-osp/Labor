...
void BooleanVector::setBit(const uint32_t index, const bool value)
{
    if (index >= numBits_)
    {
        // Расширяем вектор до нужного размера
        uint32_t newNumBits = index + 1;
        uint32_t newNumBytes = calculateNumBytes(newNumBits);
        uint8_t* newData = new uint8_t[newNumBytes];

        // Инициализируем нулями
        for (uint32_t i = 0; i < newNumBytes; i++)
        {
            newData[i] = 0;
        }
